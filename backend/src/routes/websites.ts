...
const generateSchema = z.object({
  ...
  framework: z.enum(["HTML", "NEXT"]).default("HTML"),
...
websiteRouter.post("/generate", async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    // Next.js + Tailwind is a Pro-only feature
    if (body.framework === "NEXT" && req.userPlan === "FREE") {
      res.status(403).json({
        message: "Next.js + Tailwind generation is a Pro feature. Upgrade to unlock it.",
        upgrade: true,
      });
      return;
    }
